...
class TestQueueWorker:
    ...
    async def test_start_with_exception(
        self, mock_gather, mock_create_task, queue_worker
    ):
    ...
    async def test_worker_loop_with_jobs(self, mock_settings, mock_sleep, queue_worker):
        ...
        queue_worker._process_job.assert_any_call("processing", job1, job_tracker_instance=None)
        queue_worker._process_job.assert_any_call("processing", job2, job_tracker_instance=None)
    ...
    async def test_process_job_analyze_success(self, mock_time, queue_worker):
        ...
        queue_worker.message_handler.handle_processing_message.assert_called_once_with(
            {"repo_id": "repo-456"}, None
        )

        # Verify job was completed
        queue_worker.queue_service.complete_job.assert_called_once_with(job, job_tracker_instance=None)
    ...
    async def test_process_job_unknown_type(self, mock_time, queue_worker):
        ...
        job = {"id": "job-789", "job_type": "unknown", "payload": {"data": "test"}}
...
class TestWorkerHealthMonitor:
    ...
    async def test_check_worker_health(self, health_monitor, mock_workers):
        ...
        health_monitor._check_worker_health()
    ...
    @pytest.mark.asyncio
    async def test_check_worker_health_stats_aggregation(
        self, health_monitor, mock_workers
    ):
