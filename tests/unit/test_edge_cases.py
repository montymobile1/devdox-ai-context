...
from tests.utils import TestDataFactory, MockFactory, ErrorTestHelper
...
class TestEdgeCases:
    ...
    async def test_queue_worker_with_malformed_job_data(self):
        ...
        malformed_jobs = [
            ...
            {"id": "job4", "job_type": "analyze", "payload": {"nested": {"very": {"deep": {"data": "test"}}}}},  # Very nested
        ]
    ...
    async def test_queue_service_with_extreme_message_sizes(self):
        ...
        with patch('app.infrastructure.queues.supabase_queue.PGMQueue') as mock_pgmqueue_class:
            ...
            large_payload = {
                ...
                "nested": {"deep": {"very": {"nested": {"data": "y" * 500000}}}}  # Deeply nested
            }
...
class TestErrorRecoveryScenarios:
    ...
    async def test_processing_service_recovery_from_git_failures(self):
        ...
        for failure in git_failures:
            processing_service._get_authenticated_git_client = AsyncMock(side_effect=failure)
    ...
    async def test_queue_service_recovery_from_connection_issues(self):
        ...
        with patch('app.infrastructure.queues.supabase_queue.PGMQueue') as mock_pgmqueue_class:
            ...
            mock_queue.init = AsyncMock(side_effect=[
...
class TestMemoryAndResourceManagement:
    ...
    async def test_processing_service_resource_cleanup_on_failure(self):
        ...
        processing_service.prepare_repository = AsyncMock(return_value="/tmp/test")
...
class TestRaceConditionSimulation:
        ...
        with patch('app.main.Tortoise.init') as mock_tortoise_init:
            with patch('app.main.QueueWorker') as mock_worker_class:
                with patch('app.core.config.settings') as mock_settings:
                    ...
                    await asyncio.gather(startup_task, shutdown_task, return_exceptions=True)
    ...
    async def test_concurrent_job_processing_same_queue(self):
        ...
        total_processed = sum(worker.get_stats()["jobs_processed"] for worker in workers)
