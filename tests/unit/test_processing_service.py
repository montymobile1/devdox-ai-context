...
class TestProcessingService:
    ...
    @pytest.fixture
    def processing_service(self, mock_repositories, mock_encryption_service, mock_repo_fetcher):
    ...
    def test_init(self, processing_service, mock_repositories, mock_encryption_service):
        ...
        assert processing_service.code_chunks_repository == mock_repositories['code_chunks']
        assert processing_service.base_dir == Path("app/repos")
    
    @pytest.mark.asyncio
    @patch('shutil.rmtree')
    async def test_prepare_repository_existing_path(self, mock_rmtree, processing_service):
    ...
    async def test_clone_and_process_repository_success(self, mock_git_loader, processing_service):
        ...
        result = await processing_service.clone_and_process_repository(repo_url, repo_path, branch)
    ...
    @pytest.mark.asyncio
    @patch('app.services.processing_service.GitLoader')
    async def test_clone_and_process_repository_failure(self, mock_git_loader, processing_service):
        ...
        result = await processing_service.clone_and_process_repository(repo_url, repo_path)
        
        assert result == []
    
    @pytest.mark.asyncio
    async def test_get_authenticated_git_client(self, processing_service, mock_repositories, mock_encryption_service, sample_user, sample_git_config):
        ...
        # Setup mocks
        mock_repositories['git_label'].find_by_user_and_hosting.return_value = sample_git_config
        mock_repositories['user'].find_by_user_id.return_value = sample_user
        
        with patch.object(processing_service.git_client_factory, 'create_client') as mock_create_client:
            mock_client = MagicMock()
            mock_create_client.return_value = mock_client
            
            result = await processing_service._get_authenticated_git_client(user_id, git_provider, git_token)
            
            assert result == mock_client
            mock_repositories['git_label'].find_by_user_and_hosting.assert_called_once_with(user_id, git_token, git_provider)
            ...
            mock_encryption_service.decrypt_for_user.assert_called_once_with(
                sample_git_config.token_value,
                sample_user.encryption_salt
            )
    ...
    async def test_process_files_to_chunks(self, mock_text_splitter, processing_service):
        """Test processing files to chunks"""
        files = [
            Document(page_content="def hello(): print('hello')", metadata={"source": "hello.py"}),
            Document(page_content="def world(): print('world')", metadata={"source": "world.py"})
        ]
        context_id = "ctx123"
        repo_id = "repo456"
        user_id = "user789"
        ...
        result = await processing_service._process_files_to_chunks(files, context_id, repo_id, user_id)
    ...
    async def test_create_embeddings_success(self, mock_settings, mock_together_class, processing_service):
        """Test successful embedding creation"""
        chunks = [
            Document(page_content="def hello(): pass", metadata={"file_name": "test.py", "source": "test.py"})
        ]
    ...
    @pytest.mark.asyncio
    @patch('app.services.processing_service.Together')
    async def test_create_embeddings_empty_chunks(self, mock_together_class, processing_service):
    ...
    @patch('app.services.processing_service.Together')
    @patch('app.services.processing_service.settings')
    async def test_create_embeddings_api_failure(self,mock_settings, mock_together_class, processing_service ):
    ...
    async def test_process_repository_success(self, mock_repo_class, processing_service, mock_repositories, sample_repo):
        ...
        processing_service.prepare_repository = AsyncMock(return_value=Path("/tmp/test-repo"))
        processing_service.clone_and_process_repository = AsyncMock(return_value=[
            Document(page_content="test content", metadata={"source": "test.py"})
        ])
        processing_service._process_files_to_chunks = AsyncMock(return_value=[
            Document(page_content="chunk1", metadata={"source": "test.py"})
        ])
        processing_service._create_embeddings = AsyncMock(return_value=[
    ...
    @pytest.mark.asyncio
    async def test_process_repository_repo_not_found(self, processing_service, mock_repositories):
    ...
    @pytest.mark.asyncio
    async def test_process_repository_git_client_failure(self, processing_service, mock_repositories, sample_repo):
        ...
        mock_repositories['repo'].find_by_repo_id.return_value = sample_repo
        processing_service._get_authenticated_git_client = AsyncMock(side_effect=Exception("Git auth failed"))
    ...
    @pytest.mark.asyncio
    async def test_process_repository_cloning_failure(self, processing_service, mock_repositories, sample_repo):
        ...
        processing_service._get_authenticated_git_client = AsyncMock()
        processing_service.prepare_repository = AsyncMock(return_value=Path("/tmp/test-repo"))
        processing_service.clone_and_process_repository = AsyncMock(side_effect=Exception("Clone failed"))
...
class TestProcessingServiceIntegration:
    ...
    async def test_full_processing_workflow_minimal(self,mock_repo):
        ...
        user_repo = MagicMock()
        user_repo.find_by_user_id = AsyncMock(return_value=MagicMock(encryption_salt="salt"))
        ...
        git_label_repo = MagicMock()
        git_label_repo.find_by_user_and_hosting = AsyncMock(return_value=MagicMock(token_value="token"))
        ...
        service.prepare_repository = AsyncMock(return_value=Path("/tmp/test"))
