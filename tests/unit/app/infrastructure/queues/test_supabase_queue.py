...
def sut(fake_queue):
    q = SupabaseQueue(
        host="h", port="p", user="u", password="pw", db_name="db", table_name="tbl"
    )
...
async def test_enqueue_without_delay_uses_send_and_serializes_payload_and_config(
    sut, fake_queue
):
    payload = {"a": 1}
    job_id = await sut.enqueue(
        "qA", payload, priority=7, job_type="jt", user_id="u1", config={"x": 2}
    )
    assert job_id == "1"
...
async def test_handle_max_attempts_exceeded_archives_when_attempts_ge_max(
    sut, fake_queue
):
    archived = await sut._handle_max_attempts_exceeded(
        {"attempts": 3, "max_attempts": 3}, 42, "q"
    )
...
async def test_handle_max_attempts_exceeded_no_archive_if_under_max(sut, fake_queue):
    archived = await sut._handle_max_attempts_exceeded(
        {"attempts": 1, "max_attempts": 3}, 42, "q"
    )
...
async def test_process_messages_returns_first_valid_and_continues_on_errors(
    sut, monkeypatch
):
...
async def test_dequeue_returns_none_on_empty_batch_and_uses_effective_queue(
    sut, fake_queue
):
    fake_queue._read_return = []
    out = await sut.dequeue(
        visibility_timeout=17, batch_size=3
    )  # queue_name=None -> table_name
    assert out is None
...
async def test_dequeue_logs_and_returns_none_on_exception(
    sut, fake_queue, monkeypatch, caplog
):
    async def boom(*a, **k):
...
async def test_complete_job_success_calls_tracker_completed(sut, fake_queue):
    ...
    ok = await sut.complete_job(
        {"pgmq_msg_id": 11, "id": "11", "queue_name": "q"}, job_tracker_instance=tracker
    )
    assert ok is True
...
async def test_complete_job_still_true_if_tracker_completed_raises(sut, fake_queue):
    ...
    ok = await sut.complete_job(
        {"pgmq_msg_id": 12, "id": "12", "queue_name": "q"}, job_tracker_instance=tracker
    )
    assert ok is True
    assert tracker.completed_calls == 1
...
async def test_complete_job_false_when_delete_returns_false_and_traced(sut, fake_queue):
    ...
    tracer = FakeJobTracer()
    ok = await sut.complete_job(
        {"pgmq_msg_id": 13, "id": "13", "queue_name": "q"}, job_tracer=tracer
    )
    assert ok is False
    assert (
        tracer.errors
        and "Failed to mark job 13 as completed" in tracer.errors[0]["summary"]
    )
...
async def test_complete_job_exception_path_returns_false_and_traces(sut, fake_queue):
    ...
    tracer = FakeJobTracer()
    ok = await sut.complete_job(
        {"pgmq_msg_id": 14, "id": "14", "queue_name": "q"}, job_tracer=tracer
    )
    assert ok is False
...
async def test_fail_job_missing_msg_id_returns_permanent_and_unhandled(sut):
    tracer = FakeJobTracer()
    perma, handled = await sut.fail_job(
        {"id": "X"}, RuntimeError("boom"), job_tracer=tracer
    )
...
@pytest.mark.parametrize(
    "attempts,max_attempts,expected_delay",
    [
        (1, 3, 10),
        (2, 3, 20),
        (3, 5, 40),
...
async def test_fail_job_retry_path_deletes_resends_and_calls_tracker_retry(
    sut, fake_queue, attempts, max_attempts, expected_delay, monkeypatch
):
    job_data = {
        "pgmq_msg_id": 77,
        "id": "77",
...
async def test_fail_job_retry_logs_but_still_handles_if_tracker_retry_throws(
    sut, fake_queue, monkeypatch
):
    job_data = {
        "pgmq_msg_id": 88,
        "id": "88",
    ...
    perma, handled = await sut.fail_job(
        job_data, RuntimeError("err"), job_tracker_instance=tracker, retry=True
    )
    assert (perma, handled) == (False, True)


async def test_fail_job_archive_path_calls_archive_tracker_fail_and_traces(
    sut, fake_queue
):
    job_data = {
        "pgmq_msg_id": 99,
...
async def test_fail_job_archive_path_reports_unhandled_when_archive_fails(
    sut, fake_queue
):
...
@pytest.mark.parametrize(
    "retry,attempts,max_attempts,expected",
    [
        (True, 0, 1, True),
        (True, 1, 1, False),
        (False, 1, 9, False),
...
async def test_archive_permanently_success_marks_tracker_and_tracer(sut, fake_queue):
    ...
    tracker = FakeJobTracker()
    tracer = FakeJobTracer()
    perma, handled = await sut._archive_permanently(
        queue_name="Q",
        msg_id=5,
        job_data=job_data,
        attempts=3,
...
async def test_get_queue_stats_happy_and_error(sut, fake_queue, caplog):
    ...
    assert stats == {
        "queued": 3,
        "total": 10,
        "newest_msg_age_sec": 7,
        "oldest_msg_age_sec": 99,
    }
