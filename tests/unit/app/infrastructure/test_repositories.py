...
from models_src.test_doubles.repositories.code_chunks import EMBED_DIM, FakeCodeChunksStore, StubCodeChunksStore
from models_src.test_doubles.repositories.git_label import make_fake_git_label, StubGitLabelStore

from app.core.exceptions.local_exceptions import ContextNotFoundError, DatabaseError, RepoNotFoundError
from models_src.test_doubles.repositories.api_key import FakeApiKeyStore, StubApiKeyStore
...
from app.infrastructure.database.repositories import APIKeyRepositoryHelper, CodeChunksRepositoryHelper, \
...
from models_src.test_doubles.repositories.user import FakeUserStore, make_fake_user, StubUserStore
...
class TestContextRepositoryHelper:
    ...
    async def test_create_context_wraps_exception(self):
        ...
        assert exception_constants.DB_CONTEXT_REPO_CREATE_FAILED in str(ei.value.user_message)
    ...
    async def test_update_status_ok_and_not_found_and_wrapped(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            await helper.update_status("ctx-1", "done", datetime.datetime.now(datetime.timezone.utc), 1, 2, 3)
        ...
        with pytest.raises(ContextNotFoundError):
            await helper.update_status("ctx-404", "x", datetime.datetime.now(datetime.timezone.utc), 0, 0, 0)

        # Wrapped generic exception -> DatabaseError
        stub.set_exception(StubRepoStore.update_analysis_metadata_by_id, RuntimeError("db"))
        with pytest.raises(DatabaseError) as ei:
            await helper.update_status("ctx-e", "x", datetime.datetime.now(datetime.timezone.utc), 0, 0, 0)
        assert exception_constants.DB_CONTEXT_REPO_UPDATE_FAILED in str(ei.value.user_message)
    ...
    async def test_update_repo_system_reference_ok_and_errors(self, caplog):
        ...
        # Wrapped
        stub.set_exception(StubRepoStore.update_repo_system_reference_by_id, RuntimeError("x"))
        with pytest.raises(DatabaseError) as ei:
...
class TestCodeChunksRepositoryHelper:
    async def test_store_embeddings_ok_with_fake(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            first = await helper.store_emebeddings("repo1", "u", data, commit_number="c1")
    ...
    async def test_store_embeddings_wraps_exception(self):
        ...
        assert exception_constants.DB_CODE_CHUNKS_CREATE_FAILED in str(ei.value.user_message)
    ...
    async def test_find_by_repo_ok_and_logs_empty_on_exception(self, caplog):
        ...
        await fake.save(CodeChunksRequestDTO(repo_id="A", user_id="u", content="a", embedding=None,
                                             metadata={}, file_name="a", file_path="/a", file_size=1, commit_number="c"))
        await fake.save(CodeChunksRequestDTO(repo_id="A", user_id="u", content="b", embedding=None,
                                             metadata={}, file_name="b", file_path="/b", file_size=1, commit_number="c"))
        await fake.save(CodeChunksRequestDTO(repo_id="B", user_id="u", content="c", embedding=None,
                                             metadata={}, file_name="c", file_path="/c", file_size=1, commit_number="c"))
        ...
        stub = StubCodeChunksStore()
        stub.set_exception(StubCodeChunksStore.find_all_by_repo_id_with_limit, RuntimeError("x"))
