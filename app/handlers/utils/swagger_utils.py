...
async def get_api_schema(source: SwaggerProcessingRequest, timeout: int = 30) -> Optional[str]:
    ...
    swagger_source = source.swagger_source.lower().strip()

    if not swagger_source:
        raise ValueError("Missing 'swagger_source' in source dictionary")

    if swagger_source not in ["url", "file"]:
        raise ValueError(f"Invalid swagger_source '{swagger_source}'. Must be 'url' or 'file'")

    try:
        if swagger_source == "url":
        ...
        elif swagger_source == "file":
            swagger_path = source.swagger_file_path.strip()
            if not swagger_path:
                raise ValueError("Missing 'swagger_path' for file source")
            return await _fetch_from_file(swagger_path)

    except Exception as e:
        source_info = getattr(source, 'swagger_url' if swagger_source == "url" else 'swagger_file_path', 'unknown')

        logger.error(f"Failed to get API schema from {swagger_source} source '{source_info}': {str(e)}")
        raise
